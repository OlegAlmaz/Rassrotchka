...
namespace Rassrotchka
{
    ...
    public partial class MainWindow
    {
        ...
        private readonly NedoimkaDataSet _dataSet;
        ...
        private void _asDataView_ListChanged(object sender, ListChangedEventArgs e)
        {
            LabelRowsCount.Content = _asDataView.Count;
        }
        ...
        private void Window_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            _asDataView.ListChanged += _asDataView_ListChanged;
            _filter = new FilterClass();

            var binding = new Binding("IsEnabled")
            {
                Source = UndoItem, Mode = BindingMode.OneWay
        ...
        private void Loadis()
        {
            ...
            var z = false;
            if (app.GetData("help") != null)
                z = (bool) app.GetData("help");
            if (!z)
            {
        ...
        private void UpdateCommandBinding_OnExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            {
                var result = MessageBox.Show("Обновить базу данных?", "Информация.", MessageBoxButton.YesNo
        ...
        private void DownloadCommandBinding_OnExecuted(object sender, ExecutedRoutedEventArgs e)
        {
            ...
            {
                Title = "Выбор файлов", Multiselect = false
        ...
        private void AddNewData()
        {
            ...
            if (_argument.FilePath.EndsWith(".xls") || _argument.FilePath.EndsWith(".xlsx"))
            {
                ...
                try
                {
                    ...
                    var countModifRows = _asDataView.Count;
                    _asDataView.RowStateFilter = DataViewRowState.Added;
                    var countAddedRows = _asDataView.Count;

                    if (countAddedRows + countModifRows == 0)
                    {
                        _asDataView.RowStateFilter =
                    ...
                    else
                    {
                        ...
                        _asDataView.RowStateFilter =
                            DataViewRowState.Added | DataViewRowState.ModifiedCurrent;
                        _asDataView.Sort = string.Format("{0} DESC"
                            , _dataSet.DebitPayGen.Date_DecisColumn.ColumnName);
        ...
        {
            var changes = _dataSet.HasChanges();
            if (changes)
            {
                var namDeb = _dataSet.DebitPayGen.TableName;
                var namMont = _dataSet.MonthPay.TableName;
                var dsCnang = _dataSet.GetChanges();
                if (dsCnang.Tables[namDeb].Rows.Count > 0)
                {
                    var dataRows = dsCnang.Tables[namDeb].GetErrors();
                ...
                {
                    var dataRows = dsCnang.Tables[namMont].GetErrors();
                    foreach (var dataRow in dataRows)
                    {
                        dataRow.ClearErrors();
                        dataRow.RowError = string.Empty;
        ...
        private void Item_OnChecked(object sender, RoutedEventArgs e)
        {
            ...
            {
                ItemAll, ItemAddAndModif, ItemAdd, ItemDel, ItemCur, ItemOrig, ItemUnchang
            };
        ...
        private void UndoSelectedRowsMenuItem_OnClick(object sender, RoutedEventArgs e)
        {
            ...
            {
                var selectedItems = DebitPayGenDataGrid.SelectedItems;
                var rows = (from object selectedItem in selectedItems select ((DataRowView) selectedItem).Row)
                    .ToList();
        ...
        {
            var nameMenu = ((MenuItem) sender).Name;
            var numFact = GetNumFact(nameMenu); //индекс фабрики
            LabelInfo.Content = "";
            Cursor = Cursors.Wait;
        ...
        private void SetPeiod()
        {
            ...
            if (showDialog != null && (bool) showDialog)
            {
                ...
                dict.TryGetValue(_argument.DateEnd.Month, out var namesMonth);
        ...
        private void BwOnDoWork(object sender, DoWorkEventArgs e)
        {
            ...
            var factoriesList = new List<Lazy<AbstractFactory>>
            {
                ...
                new Lazy<AbstractFactory>(() => new FactoryForecast1())
                , //1
                new Lazy<AbstractFactory>(() => new FactoryForecast20()) //2
            };
            var abstractFactory = factoriesList[numFactory].Value;
        ...
        private void CloseWindow()
        {
            if (_dataSet.HasChanges())
                MessageBox.Show("Сохраните имеющиеся изменения данных перед закрытием программы!"
                    , "Предупреждение",
